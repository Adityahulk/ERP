...
export default function PartyList() {
  ...
  const { data: statement } = usePartyStatement(
    activeTab === 'ledger' ? selectedId : undefined,
    (ledgerFrom || ledgerTo) ? { from_date: ledgerFrom || undefined, to_date: ledgerTo || undefined } : undefined,
  );
  const ledgerLines = statement?.lines || [];
                  ...
                  <CardContent className="p-4">
                    <div className="flex flex-wrap items-end justify-between gap-3 mb-4">
                      <div className="flex gap-2 items-end">
                        <div>
                          ...
                          <input type="date" value={ledgerFrom} onChange={(e) => setLedgerFrom(e.target.value)} className="border rounded-md px-2 py-1.5 text-sm" />
                        </div>
                        <div>
                          <label className="text-xs font-medium text-muted-foreground block mb-1">To</label>
                          <input type="date" value={ledgerTo} onChange={(e) => setLedgerTo(e.target.value)} className="border rounded-md px-2 py-1.5 text-sm" />
                        </div>
                      </div>
                      <Button
                        size="sm" variant="outline"
                        disabled={!ledgerLines.length}
                        onClick={() => {
                          downloadXlsx(`party-statement-${party?.name || selectedId}.xlsx`, 'Statement', ledgerLines.map((l: any) => ({
                            Date: formatDate(l.date), 'Voucher Type': l.voucher_type || '', Reference: l.reference_type || '',
                            Debit: (l.debit / 100 || 0).toFixed(2), Credit: (l.credit / 100 || 0).toFixed(2), Balance: (l.running_balance / 100 || 0).toFixed(2),
                          })));
                        }}
                      >
                        Export Excel
                      ...
                      <Button
                        ...
                        disabled={!ledgerLines.length}
                        onClick={() => {
                          const header = 'Date,Voucher Type,Reference,Debit,Credit,Balance\n';
                          const rows = ledgerLines.map((l: any) =>
                            [formatDate(l.date), l.voucher_type || '', l.reference_type || '', (l.debit / 100 || 0).toFixed(2), (l.credit / 100 || 0).toFixed(2), (l.running_balance / 100 || 0).toFixed(2)].join(',')
                          ).join('\n');
                          const blob = new Blob([header + rows], { type: 'text/csv' });
                          const url = URL.createObjectURL(blob);
                          const a = document.createElement('a');
                          a.href = url; a.download = `party-statement-${selectedId}.csv`;
                          a.click(); URL.revokeObjectURL(url);
                        }}
                      >
                        Export CSV
                      </Button>
                      <Button size="sm" variant="outline" disabled={!ledgerLines.length} onClick={() => window.print()}>
                        Print / PDF
                      </Button>
                    </div>
                    ...
                    {statement?.aging && (
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
                        <div className="border rounded-md p-2.5 text-center"><p className="text-[10px] text-muted-foreground uppercase">0-30 days</p><p className="text-sm font-bold tabular-nums">{formatMoney(statement.aging.days_0_30 || 0)}</p></div>
                        <div className="border rounded-md p-2.5 text-center"><p className="text-[10px] text-muted-foreground uppercase">31-60 days</p><p className="text-sm font-bold tabular-nums">{formatMoney(statement.aging.days_31_60 || 0)}</p></div>
                        <div className="border rounded-md p-2.5 text-center"><p className="text-[10px] text-muted-foreground uppercase">61-90 days</p><p className="text-sm font-bold tabular-nums">{formatMoney(statement.aging.days_61_90 || 0)}</p></div>
                        <div className="border rounded-md p-2.5 text-center"><p className="text-[10px] text-muted-foreground uppercase">90+ days</p><p className="text-sm font-bold tabular-nums text-red-600">{formatMoney(statement.aging.days_90_plus || 0)}</p></div>
                      </div>
                    )}

                    {!ledgerLines.length ? (
                      <p className="text-sm text-muted-foreground text-center py-10">No ledger entries in this period</p>
                    ) : (
                        ...
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="border-b bg-slate-50/70">
                              ...
                              <th className="px-4 py-2.5 text-left font-medium text-xs text-muted-foreground">Voucher Type</th>
                          ...
                          <tbody className="divide-y">
                            {ledgerLines.map((l: any, i: number) => (
                              <tr key={i}>
                                <td className="px-4 py-2.5 text-muted-foreground">{formatDate(l.date)}</td>
                                <td className="px-4 py-2.5 capitalize">{(l.voucher_type || l.reference_type || '—').replace(/_/g, ' ')}</td>
                                <td className="px-4 py-2.5 text-right tabular-nums">{l.debit > 0 ? formatMoney(l.debit) : ''}</td>
                                <td className="px-4 py-2.5 text-right tabular-nums">{l.credit > 0 ? formatMoney(l.credit) : ''}</td>
                                <td className="px-4 py-2.5 text-right tabular-nums font-semibold">{formatMoney(l.running_balance || 0)}</td>
                              </tr>
                            ))}
                          </tbody>
                          <tfoot>
                            <tr className="border-t-2 bg-slate-50/70 font-bold">
                              <td className="px-4 py-2.5" colSpan={4}>Closing Balance</td>
                              <td className="px-4 py-2.5 text-right tabular-nums">{formatMoney(statement?.closingBalance || 0)}</td>
                            </tr>
                          </tfoot>
                        </table>
                      </div>
