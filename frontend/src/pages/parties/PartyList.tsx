...
export default function PartyList() {
  ...
  const stats = [
    { label: 'Parties', value: meta.total_parties ?? 0, icon: Users, color: 'text-slate-700 bg-slate-100 dark:bg-slate-500/10' },
    {
      label: 'Receivable',
      value: formatMoney(parseInt(meta.total_receivable) || 0),
      icon: IndianRupee,
      color: 'text-emerald-600 bg-emerald-50 dark:bg-emerald-500/10',
    },
    {
      label: 'Payable',
      value: formatMoney(parseInt(meta.total_payable) || 0),
      icon: IndianRupee,
            ...
            {parties.map((p: any) => (
              <tr key={p.id} className="border-b hover:bg-muted/30 cursor-pointer" onClick={() => navigate(`/parties/${p.id}`)}>
                ...
                >
                  ...
                  {p.balance > 0 && <span className="text-[9px] block text-muted-foreground">receivable</span>}
                  {p.balance < 0 && <span className="text-[9px] block text-muted-foreground">payable</span>}
                </td>
        ...
        {pagination && pagination.totalPages > 1 && (
          <div className="flex items-center justify-between p-4 border-t">
            ...
            <div className="flex gap-2">
              <Button size="sm" variant="outline" disabled={!pagination.hasPrev} onClick={() => setFilters((f: any) => ({ ...f, page: f.page - 1 }))}>
                Previous
              </Button>
              <Button size="sm" variant="outline" disabled={!pagination.hasNext} onClick={() => setFilters((f: any) => ({ ...f, page: f.page + 1 }))}>
                Next
              </Button>
            </div>
          </div>
        ...
        <SheetContent side="right" className="overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>Phone</Label>
                <Input className="mt-1" value={form.phone || ''} onChange={(e) => u('phone', e.target.value)} />
              </div>
              <div>
                <Label>Email</Label>
                <Input className="mt-1" type="email" value={form.email || ''} onChange={(e) => u('email', e.target.value)} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>PAN</Label>
                <Input className="mt-1 uppercase font-mono" maxLength={10} value={form.pan || ''} onChange={(e) => u('pan', e.target.value.toUpperCase())} />
              </div>
            </div>
            <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                <Label>City</Label>
                <Input className="mt-1" value={form.city || ''} onChange={(e) => u('city', e.target.value)} />
              </div>
              <div>
                <Label>State</Label>
                <Input className="mt-1" value={form.state || ''} onChange={(e) => u('state', e.target.value)} />
              </div>
              <div>
                <Label>Pincode</Label>
