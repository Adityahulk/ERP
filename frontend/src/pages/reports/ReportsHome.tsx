...
export default function ReportsHome() {
  ...
  const GST_ONLY_REPORTS = new Set(['GST Out/In summary', 'GSTR-1 Data', 'GSTR-2A Reconciliation', 'GSTR2', 'GSTR-3B Summary', 'HSN Summary', 'SAC Summary', 'Input Tax Credit', 'GST Rate Report']);
  const visibleReportCategories = gstEnabled
    ? reportCategories
    : reportCategories
        .filter((cat) => cat.id !== 'gst')
        .map((cat) => ({ ...cat, reports: (cat.reports as readonly string[]).filter((r) => !GST_ONLY_REPORTS.has(r)) as any }))
        ...
        <div className="flex-1 p-4 sm:p-6 overflow-y-auto reports-print-area">
          ...
          <Card>
            <CardContent className="p-4 sm:p-6">
              {!gstEnabled && GST_ONLY_REPORTS.has(activeReport) ? (
                <div className="text-center py-12 text-muted-foreground">
                  <p className="font-medium text-foreground">GST Disabled For This Company</p>
                  <p className="text-sm mt-1">Enable GST in Settings → Taxes &amp; GST to use this report.</p>
                </div>
              ) : (
                <>
