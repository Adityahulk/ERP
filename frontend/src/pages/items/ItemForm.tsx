...
export default function ItemForm({ open, onOpenChange, item, defaultItemType = 'product' }: Props) {
  ...
  useEffect(() => {
    if (item) {
      ...
      setForm({
        ...
        mrp: (item as any).mrp ? Number(((item as any).mrp / 100).toFixed(2)) : '',
  ...
  const handleSubmit = async () => {
    ...
    const data: any = {
      ...
      mrp: form.mrp ? Math.round(form.mrp * 100) : null,
        ...
        <Tabs defaultValue="basic" className="space-y-6">
          ...
          <TabsContent value="basic" className="space-y-4">
            <div className="flex items-center justify-between rounded-lg border border-dashed p-3 bg-muted/30">
              <div>
                <p className="text-sm font-medium">AI Label Scan</p>
                <p className="text-xs text-muted-foreground">Upload a photo of the product label to auto-fill name, MRP, batch & expiry.</p>
              </div>
              <Button type="button" variant="outline" size="sm" disabled={labelScanLoading} onClick={() => labelScanInputRef.current?.click()}>
                {labelScanLoading ? 'Scanning…' : 'Upload Label'}
              </Button>
              <input
                ...
                onChange={async (e) => {
                  ...
                  try {
                    ...
                    const res = await api.post('/items/label-scan', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
                    const fields = res.data?.data;
                    if (!fields) throw new Error('No data returned');
                    if (fields.detectedName) update('name', fields.detectedName);
                    if (fields.detectedBarcode) update('barcode', fields.detectedBarcode);
                    if (fields.detectedMrp) update('mrp', fields.detectedMrp / 100);
                    toast.success(`Label scanned (${fields.confidence} confidence) — review the auto-filled fields before saving.`);
                  } catch (err: any) {
                    toast.error(err.response?.data?.error || 'Could not scan this label');
                  } finally {
            ...
            <div className={`grid grid-cols-1 gap-4 ${itemSettings.barcode_scan ? 'sm:grid-cols-3' : 'sm:grid-cols-2'}`}>
              <div>
                ...
                <div className="flex gap-1 mt-1">
                  <Input placeholder="Leave blank to auto-generate" value={form.sku || ''} onChange={e => update('sku', e.target.value)} />
                  <Button
                    variant="outline" size="icon" title="Preview next auto-generated code"
                    onClick={async () => {
                      try {
                        const res = await api.get('/items/next-assign-code');
                        update('sku', res.data?.data?.next_assign_code || '');
                      } catch {
                        toast.error('Could not fetch the next Assign Code');
                      }
                    }}
                  >
                ...
                <p className="text-[11px] text-muted-foreground mt-1">Sequential, unique, searchable, and linked to this item's barcode. Leaving it blank assigns the next number automatically when you save.</p>
              </div>
          ...
          <TabsContent value="pricing" className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex flex-col space-y-3 p-4 rounded-xl border bg-card shadow-sm hover:border-primary/30 transition-all duration-200">
                ...
                <div className="space-y-1">
                  <Label htmlFor="mrp" className="font-medium text-xs text-muted-foreground">
                    MRP <span className="opacity-70">(printed ceiling price — optional, shown struck-through on barcode labels if set)</span>
                  </Label>
                  <div className="relative mt-1">
                    <span className="absolute inset-y-0 left-0 pl-3 flex items-center text-muted-foreground font-medium text-sm">
                      {currencySymbol(form.price_currency_code)}
                    ...
                    <Input
                      id="mrp"
                      type="number"
                      className="pl-7 tabular-nums text-sm h-9 w-full"
                      min={0}
                      step={0.01}
                      placeholder="Same as selling price if left blank"
                      value={form.mrp || ''}
