...
export default function InvoiceDetail() {
  ...
  const printReceipt = async () => {
    ...
    const t = toast.loading('Sending receipt to print…');
    const ok = await printPdfFromUrl(`/print/receipt/${id}`, { width }, `thermal receipt ${width}mm, invoice ${id}`);
    if (ok) toast.success('Print dialog opened — select your thermal printer', { id: t });
  ...
  const printThermalInvoice = async () => {
    ...
    const t = toast.loading('Sending thermal invoice (50×25mm) to print…');
    const ok = await printPdfFromUrl(`/print/thermal-invoice/${id}`, undefined, `thermal invoice 50x25, invoice ${id}`);
    if (ok) toast.success('Print dialog opened — select your thermal printer', { id: t });
    else toast.dismiss(t);
      ...
      {waPickerOpen && (
        <Card className="border-emerald-200">
          <CardContent className="p-4 flex flex-wrap items-center gap-3">
            ...
            <Button
              size="sm" variant="outline" loading={twilioSending}
              onClick={async () => {
                setTwilioSending(true);
                try {
                  const res = await api.post(`/notifications/send-invoice/${id}`, sharePhone ? { phone: sharePhone } : {});
                  if (res.data?.data?.delivered) toast.success('Sent via WhatsApp Business — logged in WhatsApp → Logs');
            ...
            >
              Send via WhatsApp Business (tracked)
            </Button>
