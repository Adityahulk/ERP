...
import { Search, Loader2, Camera, Plus, Minus, Trash2, User, FileText, QrCode, PackagePlus, X, Check, Undo2, Printer, Copy, ShieldAlert, ArrowLeftRight, TrendingUp } from 'lucide-react';
...
export default function BillingScreen() {
  ...
  const [appliedCoupon, setAppliedCoupon] = useState<{ couponId: string; code: string; discountPaise: number } | null>(null);
  ...
  const [exchangeSourceInvoice, setExchangeSourceInvoice] = useState<any | null>(null);
  ...
  const { data: dashboardWidgets, isLoading: dashboardLoading } = useQuery({
    queryKey: ['pos', 'dashboard-widgets'],
    queryFn: async () => (await api.get('/pos/dashboard-widgets')).data?.data,
  ...
  const handlePrintReceipt = async (id: string) => {
    ...
    await printPdfFromUrl(`/print/receipt/${id}`, { width: w }, `POS receipt ${w}mm, invoice ${id}`);
  };
  ...
  // Real company-level GST setting — when disabled, tax is genuinely
  // excluded from the total, not just hidden from the display. A
  // hidden-but-still-calculated tax would make the printed/charged
  // total silently wrong relative to what's shown.
  const gstEnabled = companyData?.tax_settings?.enable_gst !== false;

  // Note: POS often strictly considers CGST/SGST by default. Interstate can be toggled by party selection. We assume Intra.
  const totalTax = gstEnabled ? billItems.reduce((acc, i) => acc + Math.round(i.taxable * i.gst_rate / 100), 0) : 0;
  ...
  const { data: loyaltySettingsData } = useQuery({
    queryKey: ['loyalty-settings'],
    queryFn: async () => (await api.get('/settings-ext/loyalty')).data?.data,
  });
  const loyaltyRedemptionPaise = loyaltySettingsData?.enabled ? Number(loyaltySettingsData.redemption_value_paise || 0) : 0;

  const { data: loyaltyBalanceData } = useQuery({
    queryKey: ['loyalty-balance', customerInfo.id],
    queryFn: async () => (await api.get(`/settings-ext/loyalty/${customerInfo.id}/balance`)).data?.data,
    enabled: !!customerInfo.id && !!loyaltySettingsData?.enabled,
  });
  const loyaltyBalance = loyaltyBalanceData?.balance ?? null;
  ...
  const voidBillMut = useMutation({
    mutationFn: async ({ id, reason }: { id: string; reason: string }) => api.post(`/pos/invoices/${id}/void`, { reason }),
    onSuccess: (res: any) => {
      toast.success(`${res.data?.data?.invoiceNumber} voided — stock and accounting reversed`);
      setSelectedBill((b: any) => b ? { ...b, status: 'cancelled' } : b);
      setVoidReason('');
    },
    onError: (e: any) => toast.error(e.response?.data?.error || 'Could not void this bill'),
  });

  const exchangeMut = useMutation({
    mutationFn: async () => {
      const returnItems = (Object.entries(exchangeReturnSelections) as [string, number][])
        .filter(([, qty]) => qty > 0)
        .map(([itemId, qty]) => {
          const src = exchangeSourceInvoice.items.find((it: any) => it.item_id === itemId);
          return { item_id: itemId, quantity: qty, unit_price: src.unit_price };
        });
      if (!returnItems.length) throw new Error('Select at least one item to return');
      const newItems = billItems.map((b) => ({
        item_id: b.item_id, item_name: b.name, hsn_code: b.hsn_code,
        quantity: b.quantity, unit_price: b.unit_price, gst_rate: b.gst_rate, discount_amount: b.discount_amount,
      }));
      return api.post(`/pos/invoices/${exchangeSourceInvoice.id}/exchange`, { returnItems, newItems, payment_mode: paymentMode });
    },
    onSuccess: (res: any) => {
      const d = res.data?.data;
      toast.success(`Exchange complete — new invoice ${d.newInvoiceNumber}. ${d.settlement === 'refund_due' ? `Refund due: ${formatMoney(-d.differencePaise)}` : d.settlement === 'customer_pays' ? `Collected: ${formatMoney(d.differencePaise)}` : 'No balance due.'}`, { duration: 6000 });
      setExchangeSourceInvoice(null);
      setBillItems([]);
      setQuickActionsOpen(false);
    },
    onError: (e: any) => toast.error(e.response?.data?.error || 'Exchange failed'),
  });
  ...
  const createInvoiceMut = useMutation({
    mutationFn: async () => {
      ...
      const payload: Record<string, unknown> = {
        ...
        coupon_id: appliedCoupon?.couponId || undefined,
        coupon_discount_paise: appliedCoupon?.discountPaise || undefined,
  ...
  return (
    <div className="h-[calc(100vh-7.5rem)] overflow-hidden flex flex-col gap-3">
      <div className="flex-1 flex flex-col lg:flex-row gap-3 min-h-0">
        ...
        <div className="flex-1 flex flex-col bg-background border rounded-xl overflow-hidden shadow-sm shadow-black/5 min-h-0">
          ...
          <div className="p-3 border-b flex gap-2 items-center bg-muted/20 relative z-30">
                  ...
                  {searchResults.map((item: any, idx: number) => (
                    ...
                    >
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground flex gap-2"><span>SKU: {item.sku}</span> {gstEnabled && <span>GST: {item.gst_rate}%</span>}</div>
                      </div>
            ...
            <Button size="lg" variant="outline" className="h-12 px-3 shrink-0 gap-1.5" onClick={() => setQuickActionsOpen(true)}>
              <Undo2 className="h-5 w-5" /> <span className="hidden sm:inline text-sm font-medium">Bill Actions</span>
            </Button>
            <Button size="lg" variant="outline" className="h-12 px-3 shrink-0 gap-1.5" onClick={() => setDashboardOpen(true)}>
              <TrendingUp className="h-5 w-5" /> <span className="hidden sm:inline text-sm font-medium">Today</span>
            </Button>
          </div>
                  ...
                  return (
                    <div style={style} className="grid grid-cols-[1fr_8rem_6rem_6rem_8rem_3rem] items-center border-b hover:bg-muted/50 py-2 px-4 transition-colors">
                      <div>
                        ...
                        {gstEnabled && <div className="text-xs text-muted-foreground">GST {item.gst_rate}%</div>}
                      </div>
        ...
        <Card className="p-3 shadow-sm border-2 border-transparent focus-within:border-primary/50 transition-colors shrink-0 relative">
          ...
          {isEditingCustomer ? (
            ...
            <div>
              ...
              {customerInfo.id && loyaltyBalance != null && loyaltyBalance > 0 && (
                <div className="mt-1.5 flex items-center gap-2">
                  <span className="text-xs font-medium text-amber-700 bg-amber-50 border border-amber-200 rounded-full px-2 py-0.5">{loyaltyBalance} points</span>
                  {!appliedCoupon && (
                    <button
                      type="button"
                      ...
                      onClick={async () => {
                        try {
                          const res = await api.post(`/settings-ext/loyalty/${customerInfo.id}/redeem`, { points: loyaltyBalance });
                          const redeemValue = Math.min(loyaltyBalance * loyaltyRedemptionPaise, grandTotal);
                          setDiscountTotal((d) => d + redeemValue);
                          qc.invalidateQueries({ queryKey: ['loyalty-balance', customerInfo.id] });
                          toast.success(`Redeemed ${res.data?.data?.redeemed} points (-${formatMoney(redeemValue)})`);
                        } catch (e: any) {
                          toast.error(e.response?.data?.error || 'Could not redeem points');
                        }
                      }}
                    >
                      Redeem
        ...
        <Card className="p-4 flex-1 shadow-sm flex flex-col min-h-0">
          ...
          <div className="space-y-2 flex-1 overflow-y-auto text-sm">
            ...
            <div className="flex justify-between items-center gap-2">
              {appliedCoupon ? (
                <div className="flex items-center justify-between w-full bg-emerald-50 border border-emerald-200 rounded-md px-2 py-1">
                  <span className="text-xs font-semibold text-emerald-700">{appliedCoupon.code} applied (-{formatMoney(appliedCoupon.discountPaise)})</span>
                  <button onClick={() => { setAppliedCoupon(null); setDiscountTotal((d) => Math.max(0, d - appliedCoupon.discountPaise)); }} className="text-xs text-emerald-700 hover:underline">Remove</button>
                </div>
              ) : (
                <>
                  <Input placeholder="Coupon code" className="h-7 text-xs" value={couponInput} onChange={(e) => setCouponInput(e.target.value.toUpperCase())} />
                  <Button
                    size="sm" variant="outline" className="h-7 text-xs px-2 shrink-0" disabled={!couponInput || couponLoading}
                    onClick={async () => {
                      setCouponLoading(true);
                      try {
                        const res = await api.post('/coupons/validate', { code: couponInput, purchase_amount_paise: taxable + totalTax });
                        const data = res.data?.data;
                        setAppliedCoupon(data);
                        setDiscountTotal((d) => d + data.discountPaise);
                        toast.success(`Coupon applied: -${formatMoney(data.discountPaise)}`);
                      } catch (e: any) {
      ...
      <Sheet open={quickActionsOpen} onOpenChange={(v) => { setQuickActionsOpen(v); if (!v) { setSelectedBill(null); setBillSearchQuery(''); setBillSearchResults([]); setVoidReason(''); } }}>
        <SheetContent className="w-full max-w-md overflow-y-auto">
          <SheetHeader><SheetTitle>Bill Actions</SheetTitle></SheetHeader>
            ...
            <div className="relative">
              <Input
                ...
                onChange={async (e) => {
                  ...
                  setBillSearchQuery(v);
                  setSelectedBill(null);
                  if (v.length < 2) { setBillSearchResults([]); return; }
                  setBillSearchLoading(true);
                  try {
                    const res = await api.get('/pos/invoices/lookup', { params: { q: v } });
                    setBillSearchResults(res.data?.data ?? []);
                  } finally { setBillSearchLoading(false); }
                }}
              />
              {billSearchLoading && <Loader2 className="absolute right-2.5 top-2.5 h-4 w-4 animate-spin text-muted-foreground" />}
              {!selectedBill && billSearchResults.length > 0 && (
                <div className="mt-1 border rounded-lg divide-y max-h-48 overflow-y-auto">
                  {billSearchResults.map((b: any) => (
                    <button key={b.id} className="w-full text-left px-3 py-2 hover:bg-muted text-sm flex justify-between items-center" onClick={() => { setSelectedBill(b); setBillSearchResults([]); setBillSearchQuery(b.invoice_number); }}>
                      <span>
                        <span className="font-mono font-medium">{b.invoice_number}</span>
                        <span className="text-muted-foreground ml-2">{b.party_name}</span>
                      </span>
                      <span className="font-semibold tabular-nums">{formatMoney(b.total_amount)}</span>
                    </button>
                  ))}
                </div>
              )}
              ...
              <div className="space-y-3">
                <div className="rounded-lg border p-3 bg-muted/30">
                  <div className="flex justify-between items-baseline">
                    <span className="font-mono font-semibold">{selectedBill.invoice_number}</span>
                    {selectedBill.status === 'cancelled' && <Badge className="bg-red-100 text-red-700">Voided</Badge>}
                  </div>
                  <p className="text-sm text-muted-foreground">{selectedBill.party_name} · {formatMoney(selectedBill.total_amount)}</p>
                </div>

                <div className="grid grid-cols-2 gap-2">
                  <Button variant="outline" size="sm" className="gap-1.5" onClick={() => printPdfFromUrl(`/invoices/${selectedBill.id}/pdf`, undefined, `reprint invoice ${selectedBill.id}`)}>
                    <Printer className="h-3.5 w-3.5" /> Reprint
                  </Button>

                  <Button variant="outline" size="sm" className="gap-1.5" onClick={async () => {
                    try {
                      const res = await api.get(`/pos/invoices/${selectedBill.id}/duplicate-draft`);
                      const draft = res.data?.data;
                      setBillItems((draft.items || []).map((it: any) => ({
                        item_id: it.item_id, name: it.item_name, hsn_code: it.hsn_code, unit: it.unit,
                        quantity: Number(it.quantity), unit_price: it.unit_price, discount_amount: it.discount_amount || 0,
                        gst_rate: it.gst_rate || 0, taxable: it.unit_price * it.quantity - (it.discount_amount || 0),
                      })));
                      if (draft.party?.party_id) setCustomerInfo({ id: draft.party.party_id, name: draft.party.party_name, phone: draft.party.party_phone_snapshot });
                      toast.success('Bill duplicated into cart — review and complete the sale');
                      setQuickActionsOpen(false);
                    } catch { toast.error('Could not duplicate this bill'); }
                  }}>
                    <Copy className="h-3.5 w-3.5" /> Duplicate
                  </Button>

                  <Button variant="outline" size="sm" className="gap-1.5" disabled={selectedBill.status === 'cancelled'} onClick={() => {
                    navigate(`/sales-hub/returns?invoice_id=${selectedBill.id}`);
                    setQuickActionsOpen(false);
                  }}>
                    <Undo2 className="h-3.5 w-3.5" /> Return
                  </Button>

                  <Button variant="outline" size="sm" className="gap-1.5" disabled={selectedBill.status === 'cancelled' || billItems.length === 0} title={billItems.length === 0 ? 'Add the new items to the cart first, then exchange' : ''} onClick={async () => {
                    try {
                      const res = await api.get(`/pos/invoices/${selectedBill.id}/full`);
                      setExchangeSourceInvoice(res.data?.data);
                      setExchangeReturnSelections({});
                    } catch { toast.error('Could not load this bill for exchange'); }
                  }}>
                ...
                {selectedBill.status !== 'cancelled' && (
                  <div className="border-t pt-3 space-y-2">
                    <Label className="text-xs flex items-center gap-1.5"><ShieldAlert className="h-3.5 w-3.5 text-red-600" /> Void Bill (manager approval required)</Label>
                    <Input placeholder="Reason for voiding (required)" value={voidReason} onChange={(e) => setVoidReason(e.target.value)} className="text-sm" />
                    <Button
                      variant="destructive" size="sm" className="w-full"
                      disabled={!voidReason.trim() || voidBillMut.isPending}
                      onClick={() => {
                        if (window.confirm(`Void ${selectedBill.invoice_number}? This restores stock and reverses the accounting entry. This cannot be undone.`)) {
                          voidBillMut.mutate({ id: selectedBill.id, reason: voidReason });
                        }
                      }}
                    >
                      Void This Bill
      ...
      <Sheet open={!!exchangeSourceInvoice} onOpenChange={(v) => { if (!v) setExchangeSourceInvoice(null); }}>
        <SheetContent className="w-full max-w-md overflow-y-auto">
          <SheetHeader><SheetTitle>Exchange — {exchangeSourceInvoice?.invoice_number}</SheetTitle></SheetHeader>
          <div className="mt-4 space-y-4">
            <p className="text-xs text-muted-foreground bg-blue-50 border border-blue-200 rounded-md p-2.5">
              Select what's being returned from this bill below. The <strong>{billItems.length} item(s) currently in your cart</strong> will be billed as the new items. The difference is settled automatically — extra payment collected, or a refund recorded.
            </p>
            <div className="space-y-2">
              <Label className="text-xs">Items on original bill</Label>
              {(exchangeSourceInvoice?.items || []).map((it: any) => {
                const maxQty = Number(it.quantity) - Number(it.already_returned_qty || 0);
                return (
                  <div key={it.item_id} className="flex items-center justify-between border rounded-md p-2">
                    <div>
                      <p className="text-sm font-medium">{it.item_name}</p>
                      <p className="text-xs text-muted-foreground">Qty on bill: {it.quantity} · {formatMoney(it.unit_price)}/unit</p>
                    </div>
                    <Input
                      type="number" min={0} max={maxQty} className="w-20 h-8 text-right"
                      disabled={maxQty <= 0}
                      value={exchangeReturnSelections[it.item_id] || ''}
                      placeholder="0"
                      onChange={(e) => setExchangeReturnSelections((s) => ({ ...s, [it.item_id]: Math.min(maxQty, Math.max(0, parseInt(e.target.value) || 0)) }))}
                    />
                  </div>
                );
              })}
            </div>
            <Button className="w-full" loading={exchangeMut.isPending} disabled={billItems.length === 0} onClick={() => exchangeMut.mutate()}>
              Complete Exchange
            </Button>
          </div>
        </SheetContent>
      ...
      <Sheet open={dashboardOpen} onOpenChange={setDashboardOpen}>
        <SheetContent className="w-full max-w-md overflow-y-auto">
          <SheetHeader><SheetTitle>Today's Summary</SheetTitle></SheetHeader>
          {dashboardLoading && <p className="text-sm text-muted-foreground text-center py-8">Loading…</p>}
          {!dashboardLoading && dashboardWidgets && (
            <div className="mt-4 space-y-4">
              <div className="grid grid-cols-3 gap-2">
                <div className="border rounded-lg p-2.5 text-center">
                  <p className="text-[10px] text-muted-foreground uppercase">Sales</p>
                  <p className="text-base font-bold tabular-nums">{formatMoney(dashboardWidgets.todaySalesPaise)}</p>
                </div>
                <div className="border rounded-lg p-2.5 text-center">
                  <p className="text-[10px] text-muted-foreground uppercase">Bills</p>
                  <p className="text-base font-bold tabular-nums">{dashboardWidgets.todayBillCount}</p>
                </div>
                <div className="border rounded-lg p-2.5 text-center">
                  <p className="text-[10px] text-muted-foreground uppercase">Avg Bill</p>
                  <p className="text-base font-bold tabular-nums">{formatMoney(dashboardWidgets.averageBillValuePaise)}</p>
                </div>
              </div>
              <div>
                <Label className="text-xs mb-1.5 block">Top Selling Items Today</Label>
                {(dashboardWidgets.topSellingItems || []).length === 0 && <p className="text-xs text-muted-foreground">No sales yet today.</p>}
                <div className="space-y-1">
                  {(dashboardWidgets.topSellingItems || []).map((it: any, i: number) => (
                    <div key={i} className="flex justify-between text-sm border-b py-1">
                      <span>{it.item_name}</span>
                      <span className="text-muted-foreground">{Number(it.qty_sold)} units · {formatMoney(it.revenue_paise)}</span>
                    </div>
                  ))}
                </div>
              </div>
              <div>
                <Label className="text-xs mb-1.5 block">Top Cashiers Today</Label>
                {(dashboardWidgets.topCashiers || []).length === 0 && <p className="text-xs text-muted-foreground">No sales yet today.</p>}
                <div className="space-y-1">
                  {(dashboardWidgets.topCashiers || []).map((c: any, i: number) => (
                    <div key={i} className="flex justify-between text-sm border-b py-1">
                      <span>{c.cashier_name}</span>
                      <span className="text-muted-foreground">{c.bill_count} bills · {formatMoney(c.total_paise)}</span>
                    </div>
                  ))}
                </div>
              </div>
