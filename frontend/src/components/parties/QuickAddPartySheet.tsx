...
export function QuickAddPartySheet({ open, onOpenChange, defaultName = '', onCreated }: Props) {
  ...
  const [existingParty, setExistingParty] = useState<any | null>(null);
  const [lookupLoading, setLookupLoading] = useState(false);
  const u = <K extends keyof PartyForm>(k: K, v: PartyForm[K]) => setF((p) => ({ ...p, [k]: v }));

  useEffect(() => {
    if (open) { setF(emptyForm(defaultName)); setExistingParty(null); }
  }, [open, defaultName]);

  // Real-time existing-customer detection — checks as the user types,
  // so they see "Existing Customer Found" before ever hitting submit
  // and getting a blocking duplicate error.
  useEffect(() => {
    const phone = f.phone.trim();
    if (phone.length < 6) { setExistingParty(null); return; }
    const timer = setTimeout(async () => {
      setLookupLoading(true);
      try {
        const res = await api.get('/parties/lookup-by-phone', { params: { phone } });
        setExistingParty(res.data?.data || null);
      } catch {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const err = e as { response?: { status?: number; data?: { error?: string; existingParty?: any } } };
      const existing = err.response?.data?.existingParty;
      if (err.response?.status === 409 && existing) {
        toast((t) => (
          <div className="text-sm">
            <p className="font-medium">Customer Already Exists — {existing.name}</p>
            <button className="text-indigo-600 underline text-xs mt-1" onClick={() => { onCreated(existing); onOpenChange(false); toast.dismiss(t.id); }}>
              Use Existing
            </button>
          </div>
        ), { duration: 8000 });
          ...
          <TabsContent value="essentials" className="mt-0 space-y-4 data-[state=inactive]:hidden">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                {lookupLoading && <p className="text-xs text-muted-foreground mt-1">Checking…</p>}
                {existingParty && (
                  <div className="mt-2 rounded-md border border-emerald-200 bg-emerald-50 p-2.5 text-xs space-y-1.5">
                    <p className="font-semibold text-emerald-800">✓ Existing Customer Found</p>
                    <p className="text-emerald-700">{existingParty.name} · Balance {((existingParty.balance || 0) / 100).toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}</p>
                    <p className="text-emerald-600">{existingParty.last_invoice_date ? `Last invoice: ${new Date(existingParty.last_invoice_date).toLocaleDateString('en-IN')}` : 'No invoices yet'}</p>
                    <Button
                      size="sm" type="button" className="h-7 text-xs mt-1"
                      onClick={() => { onCreated(existingParty); onOpenChange(false); }}
                    >
