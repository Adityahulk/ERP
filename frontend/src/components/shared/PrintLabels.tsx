...
export default function PrintLabels({ selectedItems, onClose }: PrintLabelsProps) {
  ...
  const [labelPreset, setLabelPreset] = useState<'tiny' | 'single' | 'double'>('single');
  const [loading, setLoading] = useState(false);
  const [previewLoading, setPreviewLoading] = useState(false);
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);

  const size: '50x24' | '58x40' | '100x50' | 'a4' = mode === 'general_printer'
    ? 'a4'
    : labelPreset === 'tiny'
      ? '50x24'
      : labelPreset === 'single'
  ...
  const buildPayload = () => ({
    mode,
    size,
    labels_per_page: mode === 'general_printer' ? Number(generalPreset) : labelPreset === 'double' ? 2 : 1,
          ...
          <CardContent className="p-6 space-y-6">
             ...
             <div className="flex gap-4 mb-4 border-b pb-4">
               {mode === 'general_printer' ? (
                 ...
                 <>
                   <button onClick={() => setLabelPreset('tiny')} className={`flex-1 p-3 border rounded-lg text-center ${labelPreset === 'tiny' ? 'bg-indigo-50 border-indigo-500 text-indigo-700 font-semibold' : 'hover:bg-slate-50'}`}>
                     1 / page <span className="block text-xs font-normal mt-1 opacity-70">50×24 mm</span>
                   </button>
