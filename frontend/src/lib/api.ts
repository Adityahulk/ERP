...
/**
 * Use this instead of `err.response?.data?.error || 'Something failed'`
 * in catch blocks. That pattern silently collapses two very different
 * failures into the same generic message:
 *   - the server responded with a real error (validation, conflict, etc.)
 *   - the request never got a response at all (backend unreachable,
 *     wrong port, CORS block, DNS failure)
 * The second case is a connection problem, not a "registration failed"
 * problem, and telling the user to "try again" is actively misleading —
 * retrying won't help if the backend isn't reachable at all.
 */
export function describeApiError(err: any, fallback = 'Something went wrong. Please try again.'): string {
  ...
  // No `response` object at all → the request never completed a round
  // trip. Surface the actual target URL so this is debuggable from the
  // error message itself instead of requiring devtools.
  const target = `${apiBaseURL}`;
  if (err?.code === 'ECONNABORTED' || err?.message?.includes('timeout')) {
    return `The server at ${target} took too long to respond. Check that the backend is running.`;
  }
  return `Could not reach the server at ${target}. Check that the backend is running and reachable, and that VITE_API_BASE_URL / VITE_BACKEND_PORT (if set) point to the right place.`;
}
