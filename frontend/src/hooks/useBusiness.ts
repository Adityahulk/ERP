...
export function usePartyStatement(partyId: string | undefined, dateRange?: { from_date?: string; to_date?: string }) {
  const validId = isValidUuid(partyId);
  return useQuery({
    queryKey: ['parties', partyId, 'statement', dateRange?.from_date, dateRange?.to_date],
    queryFn: () => api.get('/reports/party-statement-summary', { params: { party_id: partyId, ...dateRange } }).then((r) => ({
      lines: Array.isArray(r.data?.data) ? r.data.data : [],
      party: r.data?.meta?.party || null,
      aging: r.data?.meta?.aging || null,
      closingBalance: r.data?.meta?.closingBalance ?? 0,
    })),
    enabled: !!validId,
  });
}
