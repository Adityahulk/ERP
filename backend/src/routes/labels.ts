...
const VALID_TEMPLATES = new Set(['50x24', '58x40', '80x50', '100x50', '116x40', '100x100', '4x6in', 'a4']);
...
router.post('/bulk', async (req: Request, res: Response) => {
   try {
     ...
     const [itemDetails, companyRes] = await Promise.all([
       query(
         `SELECT id, name, barcode, sku, hsn_code, selling_price, mrp, gst_rate
          FROM items
     ...
     const pdfBuffer = await generateLabelsPDF(template as '50x24' | '58x40' | '80x50' | '100x50' | '116x40' | '100x100' | 'a4', printQueue, {
