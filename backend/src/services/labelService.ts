...
import { generateScannableBarcode } from './barcodeQuality';

async function getBarcodeDataUri(barcodeText: string): Promise<{ uri: string; widthMm: number; heightMm: number; warning?: string }> {
  try {
    // Real, length-aware sizing instead of a fixed low scale — a fixed
    // scale was the actual cause of "prints fine but won't scan": the
    // native bar resolution was too low once stretched to fill the
    // label via CSS, and a fixed scale produces a different (and for
    // short codes, often too-narrow) module width depending on text
    // length. Target physical size aims for the brief's stated
    // minimums (35mm+ wide, 15mm+ tall) while staying within what a
    // 50mm label can actually hold.
    const result = await generateScannableBarcode(barcodeText || 'N/A', { targetWidthMm: 38, targetHeightMm: 15 });
    if (result.quality.warning) {
      console.warn(`[barcode quality] ${barcodeText}: ${result.quality.warning}`);
    }
    return {
      uri: result.dataUri,
      widthMm: result.quality.estimatedPhysicalWidthMm,
      heightMm: result.quality.estimatedPhysicalHeightMm,
      warning: result.quality.warning,
    };
  } catch {
    return { uri: '', widthMm: 0, heightMm: 0 };
  }
}
...
const PRINTER_PROFILES = {
  LABEL_50x24: { width: 50, height: 24 },
...
function labelCard(
  ...
  const priceHtml = (!isBarcodeOnly && rPrice) ? `
    <div class="label-price" style="${rPrice.css}; font-size: ${getPriceFontSize(rPrice.text)}">
      ${i.mrp != null && Number(i.mrp) > 0 ? `<span style="font-size: 2.4mm; font-weight: normal; text-decoration: line-through; color: #888; margin-right: 1mm;">MRP ₹${(Number(i.mrp) / 100).toFixed(2)}</span>` : ''}
      ${esc(rPrice.text)}
    </div>` : '';
  ...
  const barcodeWidthCss = i.barcodeWidthMm ? `${i.barcodeWidthMm}mm` : 'auto';
  const barcodeHeightCss = i.barcodeHeightMm ? `${i.barcodeHeightMm}mm` : '15mm';
  const barcodeHtml = showBc ? `
    <div class="barcode-wrapper">
      ${i.barcodeUri ? `<img src="${i.barcodeUri}" style="display: block; width: ${barcodeWidthCss}; height: ${barcodeHeightCss}; max-width: 100%; margin: 0 auto; image-rendering: pixelated; image-rendering: crisp-edges;" />` : ''}
...
function renderDynamicTemplateCard(
  ...
  const priceHtml = (!isBarcodeOnly && rPrice) ? `
    <div class="label-price" style="${rPrice.css}; font-size: ${getPriceFontSize(rPrice.text)}">
      ${item.mrp != null && Number(item.mrp) > 0 ? `<span style="font-size: 2.4mm; font-weight: normal; text-decoration: line-through; color: #888; margin-right: 1mm;">MRP ₹${(Number(item.mrp) / 100).toFixed(2)}</span>` : ''}
      ${esc(rPrice.text)}
    </div>` : '';
  ...
  const barcodeWidthCss2 = item.barcodeWidthMm ? `${item.barcodeWidthMm}mm` : 'auto';
  const barcodeHeightCss2 = item.barcodeHeightMm ? `${item.barcodeHeightMm}mm` : '15mm';
  const barcodeHtml = showBc ? `
    <div class="barcode-wrapper">
      ${barcodeUri ? `<img src="${barcodeUri}" style="display: block; width: ${barcodeWidthCss2}; height: ${barcodeHeightCss2}; max-width: 100%; margin: 0 auto; image-rendering: pixelated; image-rendering: crisp-edges;" />` : ''}
...
const generateHtml = (
  ...
  type: '50x24' | '58x40' | '80x50' | '100x50' | '116x40' | '100x100' | '4x6in' | 'a4',
  ...
  if (type === '50x24') {
    body = items.map(i =>
      `<div class="roll-page single-50x24">${renderCard(i, '100%', '100%', 'tight')}</div>`
    ).join('');
  } else if (type === '58x40') {
    body = items.map(i =>
    ...
    body = items.map(i =>
      `<div class="roll-page single-4x6in">${renderCard(i, '100%', '100%', 'roomy')}</div>`
    ).join('');
  } else if (type === '116x40') {
    const pages: string[] = [];
...
<html><head><meta charset="utf-8"><style>
  * {
  ...
  .single-50x24 { width: 50mm; height: 24mm; display: flex; align-items: center; justify-content: center; }
  ...
  .single-4x6in { width: 101.6mm; height: 152.4mm; display: flex; align-items: center; justify-content: center; }
...
export async function generateLabelsPDF(
  template: '50x24' | '58x40' | '80x50' | '100x50' | '116x40' | '100x100' | '4x6in' | 'a4',
  itemsData: any[],
  ...
  const itemsWithBarcodes = await Promise.all(
    itemsData.map(async (item) => {
      if (!item.sku) return { ...item, barcodeUri: '', barcodeWidthMm: 0, barcodeHeightMm: 0 };
      const result = await getBarcodeDataUri(item._smart_barcode_str || item.sku);
      return { ...item, barcodeUri: result.uri, barcodeWidthMm: result.widthMm, barcodeHeightMm: result.heightMm, barcodeWarning: result.warning };
    })
  );
  ...
  if (template === '50x24') {
    width = `${PRINTER_PROFILES.LABEL_50x24.width}mm`;
    height = `${PRINTER_PROFILES.LABEL_50x24.height}mm`;
  } else if (template === '58x40') {
