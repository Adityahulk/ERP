...
function getTransporter(): Transporter | null {
  ...
  transporter = nodemailer.createTransport({
    ...
    // Without these, a slow or unreachable SMTP server can hang this
    // connection far longer than a reverse proxy's read timeout — the
    // client sees a 502 while Node is still stuck waiting on SMTP.
    // Fail fast instead: 8s to connect, 8s for the SMTP greeting,
    // 10s of socket inactivity during the actual send.
