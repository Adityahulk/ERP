...
function resolvePaperSize(paperSize: string | undefined): { cssSize: string; pdfFormat?: 'A4' | 'A5' | 'Letter'; widthMm?: number; heightMm?: number } {
  switch (paperSize) {
    case 'A5': return { cssSize: 'A5', pdfFormat: 'A5' };
    case 'Letter': return { cssSize: 'Letter', pdfFormat: 'Letter' };
    case 'Half-A4': return { cssSize: '148mm 105mm', widthMm: 148, heightMm: 105 };
    default: return { cssSize: 'A4', pdfFormat: 'A4' };
  }
}
...
function resolvePrintSettings(company: any): PrintSettings {
  ...
  // Real GST disable — reuses the existing tax_settings.enable_gst
  // field (no new setting). When false, this forces the same flags
  // that totalsRows/invoiceItemTable/printAwareCompany already check,
  // rather than adding a second, parallel hiding mechanism.
  const gstEnabled = parseObject(company?.tax_settings)?.enable_gst !== false;

  const itemTableColumns = Array.isArray(raw?.item_table?.columns) && raw.item_table.columns.length
  ...
  return {
    ...
    header: { ...DEFAULT_PRINT_SETTINGS.header, ...parseObject(raw.header), ...(gstEnabled ? {} : { gstin: false }) },
    item_table: {
      ...DEFAULT_PRINT_SETTINGS.item_table,
      ...parseObject(raw.item_table),
      columns: gstEnabled ? itemTableColumns : itemTableColumns.filter((c: string) => c !== 'gst_rate' && c !== 'tax_amount'),
    },
    layout_colors: parseObject(raw.layout_colors) as Record<string, string>,
    totals: { ...DEFAULT_PRINT_SETTINGS.totals, ...parseObject(raw.totals), ...(gstEnabled ? {} : { tax_details: false }) },
...
function buildReferenceTaxInvoiceHtml(args: {
  ...
  const itemRows = items.map((it, index) => {
    const taxLabelBlock = (index === 0 && printSettings.totals.tax_details !== false)
      ? `<div class="ref-tax-lines"><b>CGST</b><b>SGST</b>${Number(invoice.round_off || 0) ? '<b>Round Off</b>' : ''}</div>`
      : '';
    const taxAmountBlock = (index === 0 && printSettings.totals.tax_details !== false)
    ...
    <main class="ref-page">
      ...
      <table class="ref-grid ref-top-grid"><tr>
        <td style="width:50%;padding:0">
          <div class="ref-left-block ref-seller"><div class="ref-firm">${escapeHtml(sellerName)}</div>${addressHtml(companyAddress(company))}<br/>${printSettings.totals.tax_details !== false ? `GSTIN/UIN: ${escapeHtml(sellerGstin)}<br/>` : ''}State Name : ${escapeHtml(sellerStateName)}, Code : ${escapeHtml(sellerStateCode)}</div>
          <div class="ref-left-block ref-consignee"><div class="ref-party-title">Consignee (Ship to)</div><div class="ref-party-name">${escapeHtml(buyerName)}</div>${addressHtml(shippingAddress)}<br/>${printSettings.totals.tax_details !== false ? `GSTIN/UIN : ${escapeHtml(buyerGstin)}<br/>` : ''}State Name : ${escapeHtml(buyerStateName)}, Code : ${escapeHtml(buyerStateCode)}</div>
      ...
      ${printSettings.totals.tax_details !== false ? referenceTaxSummary(items, invoice) : ''}
      ${printSettings.totals.tax_details !== false ? `<div class="ref-words">Tax Amount (in words) : <b>INR ${escapeHtml(taxWords)}</b></div>` : ''}
...
function buildInvoiceHtml(args: {
  ...
  <style>
    @page{size:${resolvePaperSize(printSettings.regular.paper_size).cssSize};margin:8mm}
...
export async function generateInvoicePDF(
  ...
  const resolvedPaper = resolvePaperSize(effectivePrintSettings.regular.paper_size);
  const pdf = await page.pdf({
    ...(resolvedPaper.pdfFormat
      ? { format: resolvedPaper.pdfFormat }
      : { width: `${resolvedPaper.widthMm}mm`, height: `${resolvedPaper.heightMm}mm` }),
...
export async function generateThermalReceipt(
  ...
  company: any,
  items: any[],
  widthMm: 58 | 80 | 112 = 80,
): Promise<Buffer> {
  const paperW = widthMm === 58 ? '58mm' : widthMm === 112 ? '112mm' : '80mm';
  ...
  const gstinLine = (gstEnabled && company.gstin) ? `<div class="center small">GSTIN: ${escapeHtml(company.gstin)}</div>` : '';
  ...
  if (isDeposit) {
    ...
    const itemRowsHtml = items
      .map((it) => {
        ...
        const gst = (gstEnabled && it.gst_rate !== undefined) ? `GST: ${it.gst_rate}%` : '';
    ...
    bodySection = `
      ...
      ${(gstEnabled && cgst > 0) ? `<div class="row"><span>CGST @ ${rateGuess}%</span><span>₹${fmtPaise(cgst)}</span></div>` : ''}
      ${(gstEnabled && sgst > 0) ? `<div class="row"><span>SGST @ ${rateGuess}%</span><span>₹${fmtPaise(sgst)}</span></div>` : ''}
      ${(gstEnabled && igst > 0) ? `<div class="row"><span>IGST</span><span>₹${fmtPaise(igst)}</span></div>` : ''}
      <div class="divider"></div>
  ...
  if (thermal.barcode_or_qr === 'barcode' && invoice.invoice_number) {
    try {
      const { generateScannableBarcode } = await import('./barcodeQuality');
      const result = await generateScannableBarcode(invoice.invoice_number, { targetWidthMm: 45, targetHeightMm: 16 });
      if (result.quality.warning) console.warn(`[barcode quality] thermal invoice ${invoice.invoice_number}: ${result.quality.warning}`);
      barcodePng = result.dataUri;
    } catch {
      barcodePng = '';
    }
  }
  ...
  if (thermal.barcode_or_qr === 'barcode' && barcodePng) {
    digitalLookupSection = `<div class="bc" style="margin-top: 3mm;"><img src="${barcodePng}" alt="barcode" style="width: 45mm; height: 16mm; max-width: 100%; image-rendering: pixelated; image-rendering: crisp-edges;"/></div>`;
...
export async function generateThermalInvoicePDF(
  ...
  try {
    ...
    const { generateScannableBarcode } = await import('./barcodeQuality');
    const result = await generateScannableBarcode(barcodePayload, { targetWidthMm: 35, targetHeightMm: 15 });
    if (result.quality.warning) console.warn(`[barcode quality] thermal stub ${barcodePayload}: ${result.quality.warning}`);
    barcodeImg = result.dataUri;
  } catch {
    barcodeImg = '';
  }
