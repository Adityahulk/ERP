...
export async function postSalesInvoiceAccounting(db: Queryable, companyId: string, invoice: any, createdBy?: string | null) {
  ...
  // Lazy creation — only touches the chart of accounts for a GST
  // ledger that's actually used by this invoice. A GST-disabled
  // company (amounts always 0) never creates these accounts at all,
  // so they genuinely never appear anywhere, not just with a hidden
  // zero balance.
  const outCgst = cgstAmount > 0 ? await getOrCreateDefaultAccount(db, companyId, 'Output CGST', 'liability', 'Current Liabilities') : null;
  const outSgst = sgstAmount > 0 ? await getOrCreateDefaultAccount(db, companyId, 'Output SGST', 'liability', 'Current Liabilities') : null;
  const outIgst = igstAmount > 0 ? await getOrCreateDefaultAccount(db, companyId, 'Output IGST', 'liability', 'Current Liabilities') : null;
  const outCess = cessAmount > 0 ? await getOrCreateDefaultAccount(db, companyId, 'Output Cess', 'liability', 'Current Liabilities') : null;
  ...
  return postJournalEntry(db, {
    ...
    lines: [
      ...
      ...(outCgst ? [{ accountId: outCgst, credit: cgstAmount }] : []),
      ...(outSgst ? [{ accountId: outSgst, credit: sgstAmount }] : []),
      ...(outIgst ? [{ accountId: outIgst, credit: igstAmount }] : []),
      ...(outCess ? [{ accountId: outCess, credit: cessAmount }] : []),
      roundDifference > 0
...
export async function postPurchaseInvoiceAccounting(db: Queryable, companyId: string, invoice: any, createdBy?: string | null) {
  ...
  // Lazy creation — same reasoning as the sales side: a GST-disabled
  // purchase never creates these accounts at all.
  const inCgst = cgstAmount > 0 ? await getOrCreateDefaultAccount(db, companyId, 'Input CGST', 'asset', 'Current Assets') : null;
  const inSgst = sgstAmount > 0 ? await getOrCreateDefaultAccount(db, companyId, 'Input SGST', 'asset', 'Current Assets') : null;
  const inIgst = igstAmount > 0 ? await getOrCreateDefaultAccount(db, companyId, 'Input IGST', 'asset', 'Current Assets') : null;
  const inCess = cessAmount > 0 ? await getOrCreateDefaultAccount(db, companyId, 'Input Cess', 'asset', 'Current Assets') : null;
  ...
  return postJournalEntry(db, {
    ...
    lines: [
      ...
      ...(inCgst ? [{ accountId: inCgst, debit: cgstAmount }] : []),
      ...(inSgst ? [{ accountId: inSgst, debit: sgstAmount }] : []),
      ...(inIgst ? [{ accountId: inIgst, debit: igstAmount }] : []),
      ...(inCess ? [{ accountId: inCess, debit: cessAmount }] : []),
      roundDifference > 0
