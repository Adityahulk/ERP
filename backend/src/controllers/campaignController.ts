...
export async function createCampaign(req: Request, res: Response) {
  try {
    const companyId = req.user!.company_id;
    const { channel, name, subject, message, segment, recipients: explicitRecipients } = req.body;
    ...
    let recipients: { id: string | null; contact: string }[];
    if (Array.isArray(explicitRecipients) && explicitRecipients.length) {
      // From CSV upload or manual customer selection — validate each
      // contact looks like a real phone/email rather than trusting
      // arbitrary uploaded data outright.
      const contactPattern = channel === 'whatsapp' ? /^[0-9+][0-9\s-]{7,}$/ : /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      recipients = explicitRecipients
        .map((r: any) => ({ id: r.party_id || r.id || null, contact: String(r.contact || r.phone || r.email || '').trim() }))
        .filter((r: any) => contactPattern.test(r.contact));
      if (!recipients.length) return res.status(400).json(error(`None of the uploaded rows had a valid ${channel === 'whatsapp' ? 'phone number' : 'email address'}.`));
    } else {
      recipients = await resolveSegment(companyId, channel, segment || 'all');
