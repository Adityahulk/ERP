...
// ── POST /api/settings-ext/loyalty/:partyId/redeem ──────────────────
// Real deduction — writes a negative ledger entry immediately, so the
// balance shown anywhere else reflects the redemption right away
// rather than only after the invoice is saved.
export async function redeemLoyaltyPoints(req: Request, res: Response) {
  try {
    ...
    const balanceRes = await query(
      `SELECT COALESCE(SUM(points), 0)::int AS balance FROM loyalty_ledger WHERE company_id = $1 AND party_id = $2`,
      [companyId, req.params.partyId],
    );
    const available = balanceRes.rows[0].balance;
    if (points > available) return res.status(400).json(error(`Only ${available} points available`));

    await query(
      `INSERT INTO loyalty_ledger (company_id, party_id, points, reason) VALUES ($1,$2,$3,'Redeemed at checkout')`,
      [companyId, req.params.partyId, -points],
    );
    res.json(success({ redeemed: points, remainingBalance: available - points }));
  } catch (err: any) { res.status(500).json(error(err.message)); }
}
