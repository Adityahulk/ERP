...
async function resolveOpeningStockGodown(
  ...
  if (stockRes.rows[0]?.godown_id) return stockRes.rows[0].godown_id;

  // Never fail item creation/update just because no godown was picked
  // and this item has no stock history yet. If the company already
  // has a godown, use it (the default one, or the oldest). If they
  // have none at all, create one — this is exactly the gap that used
  // to surface as "Select a godown before setting opening stock".
  const existingGodowns = await client.query(
    `SELECT id FROM godowns WHERE company_id = $1 AND is_active = true ORDER BY is_default DESC, created_at ASC LIMIT 1`,
    [companyId],
  );
  if (existingGodowns.rows[0]?.id) return existingGodowns.rows[0].id;

  const created = await client.query(
    `INSERT INTO godowns (company_id, name, is_default, is_active) VALUES ($1, 'Default Godown', true, true) RETURNING id`,
    [companyId],
  );
...
export async function createItem(req: Request, res: Response) {
  try {
    ...
    // SKU / Assign Code — auto-generate the next sequential numeric code
    // when none is provided (manual override still works: if the
    // caller supplies one, it's used as-is and validated for uniqueness
    // exactly as before).
    if (!data.sku) {
      const seq = await query(
        `UPDATE companies SET next_assign_code = next_assign_code + 1 WHERE id = $1 RETURNING next_assign_code`,
        [companyId],
      );
      data.sku = String(seq.rows[0].next_assign_code);
    }
    ...
    if (data.sku) {
      ...
      if (dup.rows.length) return res.status(400).json(error('An item with this Assign Code already exists'));
    }
    if (data.barcode) {
      const dup = await query(
    ...
    const result = await withTransaction(async (client) => {
      ...
      const itemRes = await client.query(
        `INSERT INTO items (
          ...
          purchase_price, selling_price, mrp, price_currency_code,
        ...
        [
          ...
          normalizedPurchasePrice, normalizedSellingPrice, data.mrp || null,
      ...
      if (openingStock > 0 && trackInventory) {
        const godownId = await resolveOpeningStockGodown(client, companyId, item.id, data.godown_id, req.user!.godown_id);
...
// ── GET /api/items/next-assign-code ────────────────────────────
// Preview only — does NOT increment the counter. The real value is
// claimed atomically at create time so two simultaneous new-item forms
// can never collide on the same code.
export async function peekNextAssignCode(req: Request, res: Response) {
  try {
    const result = await query(`SELECT next_assign_code FROM companies WHERE id = $1`, [req.user!.company_id]);
    res.json(success({ next_assign_code: String(result.rows[0]?.next_assign_code ?? 1) }));
  } catch (err: any) {
    res.status(500).json(error(err.message));
  }
}
