...
import { generateQuotationPDF, generateThermalReceipt, generateThermalInvoicePDF } from '../services/pdfService';

/** GET /api/print/receipt/:invoiceId?width=58|80|112 */
export async function getReceiptPdf(req: Request, res: Response) {
  try {
    const { invoiceId } = req.params;
    const w = req.query.width === '58' ? 58 : req.query.width === '112' ? 112 : 80;
    ...
    const pdfBuffer = await generateThermalReceipt(invRes.rows[0], companyRes.rows[0], itemsRes.rows, w as 58 | 80 | 112);
...
/** GET /api/print/thermal-invoice/:invoiceId — 50mm x 25mm Thermal Invoice with CODE-128 barcode */
export async function getThermalInvoicePdf(req: Request, res: Response) {
  try {
    const { invoiceId } = req.params;
    ...
    const pdfBuffer = await generateThermalInvoicePDF(invRes.rows[0], companyRes.rows[0], itemsRes.rows);
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `inline; filename=thermal-invoice-${invRes.rows[0].invoice_number}.pdf`);
    res.send(pdfBuffer);
  } catch (err: any) {
    res.status(500).json(error(err.message));
  }
