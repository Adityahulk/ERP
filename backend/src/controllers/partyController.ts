  ...
  try {
    ...
    if (d.phone) {
      const dup = await query(
        `SELECT id, name, phone, gstin, balance, credit_limit, party_type FROM parties WHERE company_id = $1 AND phone = $2 AND is_deleted = false`,
        [companyId, d.phone],
      );
      if (dup.rows.length) {
        return res.status(409).json({ success: false, error: 'A party with this phone number already exists', existingParty: dup.rows[0] });
      }
    }
...
// ── GET /api/parties/lookup-by-phone?phone= ─────────────────────────
// Real-time existing-customer detection — called as the user types a
// phone number, before they ever try to submit a new-party form. Lets
// the UI offer "Use Existing Customer" instead of blocking with a
// duplicate error after the fact.
export async function lookupPartyByPhone(req: Request, res: Response) {
  try {
    const companyId = req.user!.company_id;
    const phone = String(req.query.phone || '').trim();
    if (!phone || phone.length < 6) return res.json(success(null));

    const result = await query(
      `SELECT p.id, p.name, p.phone, p.gstin, p.balance, p.credit_limit, p.billing_address, p.party_type,
              (SELECT MAX(invoice_date) FROM invoices i WHERE i.party_id = p.id AND i.company_id = p.company_id AND i.is_deleted = false) AS last_invoice_date
       FROM parties p
       WHERE p.company_id = $1 AND p.phone = $2 AND p.is_deleted = false
       LIMIT 1`,
      [companyId, phone],
    );
...
// ── GET /api/parties/search-suggest?q= ───────────────────────────────
// Instant suggestions while typing phone, name, or GSTIN — distinct
// from lookupPartyByPhone (exact match for the duplicate-prevention
// flow) since this returns multiple partial matches for a dropdown.
export async function searchPartySuggestions(req: Request, res: Response) {
  try {
    const companyId = req.user!.company_id;
    const q = String(req.query.q || '').trim();
    if (q.length < 2) return res.json(success([]));
    const result = await query(
      `SELECT id, name, phone, gstin, balance, party_type
       FROM parties
       WHERE company_id = $1 AND is_deleted = false
         AND (phone ILIKE $2 OR name ILIKE $2 OR gstin ILIKE $2)
