...
export async function createInvoice(req: Request, res: Response) {
  try {
    ...
    const result = await withTransaction(async (client) => {
      ...
      if (d.coupon_id && d.coupon_discount_paise) {
        const { recordCouponRedemption } = await import('./couponController');
        await recordCouponRedemption(client, companyId, d.coupon_id, invoice.id, Number(d.coupon_discount_paise));
      }

      // Loyalty points — real earning, only for an identified party (not
      // Walk-in), only when the company has actually enabled it.
      if (invoice.party_id) {
        const loyaltyRes = await client.query(`SELECT * FROM loyalty_settings WHERE company_id = $1 AND enabled = true`, [companyId]);
        if (loyaltyRes.rows.length) {
          const ls = loyaltyRes.rows[0];
          const pointsEarned = Math.floor((Number(invoice.total_amount) / 100) * Number(ls.points_per_rupee) / 100);
          if (pointsEarned > 0) {
            await client.query(
              `INSERT INTO loyalty_ledger (company_id, party_id, invoice_id, points, reason) VALUES ($1,$2,$3,$4,'Earned on sale')`,
              [companyId, invoice.party_id, invoice.id, pointsEarned],
            );
          }
        }
      }
...
export async function getInvoicePDF(req: Request, res: Response) {
  try {
    ...
    if (useThermal) {
      // Real width preference — previously this always called the
      // 50x25-locked function no matter what printer the user actually
      // had, so a 58mm/80mm printer would get the wrong-sized output.
      const widthOverride = String(req.query.width || '');
      const storedWidth = (companyForPdf as any)?.print_settings?.default_thermal_width;
      const width = ['50x25', '58', '80', '112'].includes(widthOverride) ? widthOverride : (storedWidth || '80');

      const thermalBuffer = width === '50x25'
        ? await generateThermalInvoicePDF(invRes.rows[0], companyForPdf, itemsRes.rows)
        : await generateThermalReceipt(invRes.rows[0], companyForPdf, itemsRes.rows, width === '58' ? 58 : width === '112' ? 112 : 80);
