...
export async function trialBalance(req: Request, res: Response) {
  try {
    ...
    const byId = new Map<string, any>(result.rows.map((r: any) => [r.id, { ...r, children: [] as any[] }]));
    const roots: any[] = [];
...
export async function partyStatementSummary(req: Request, res: Response) {
  try {
    ...
    const where: string[] = ['company_id = $1', 'party_id = $2'];
    const params: any[] = [companyId, party_id];
